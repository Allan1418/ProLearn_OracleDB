...
-- Se creo la vista de las USUARIOS_CURSOS que estan en estado 1

CREATE OR REPLACE VIEW FIDE_PROLEARN_FINAL_PROF.FIDE_USUARIOS_CURSOS_V AS
 SELECT 
    USUARIOS_CURSOS_TB_ID_PK,
    ID_USER,
    ID_CURSO
  FROM  FIDE_PROLEARN_FINAL_PROF.FIDE_USUARIOS_CURSOS_TB
  WHERE ESTADO_DELET = 1;
/
  ...
  BEGIN
    ...
    IF P_ID_USUARIO = 0 THEN
      INSERT INTO FIDE_USUARIOS_TB (
        ...
        P_PASSWORD,
        1
      ) RETURNING USUARIOS_TB_ID_USER_PK INTO P_ID_RESULTANTE;
    ELSE
      UPDATE FIDE_USUARIOS_TB
      SET NOMBRE = P_NOMBRE,
...
CREATE OR REPLACE PROCEDURE FIDE_PROLEARN_FINAL_PROF.USUARIO_CAMBIO_ROL_ADMIN_SP(
  P_ID_USUARIO IN NUMBER,
  P_ROL_ID IN NUMBER
) AS
BEGIN
  ...
  IF P_ROL_ID != 3 THEN
  UPDATE FIDE_USUARIOS_TB
  SET VENCIMIENTO_SUSCRIPCION = NULL
  WHERE USUARIOS_TB_ID_USER_PK = P_ID_USUARIO;
  
  ELSE
  UPDATE FIDE_USUARIOS_TB
  SET VENCIMIENTO_SUSCRIPCION = TO_DATE('01-01-2099', 'DD-MM-YYYY')
  WHERE USUARIOS_TB_ID_USER_PK = P_ID_USUARIO;
