...
AS
  V_ROL_ID NUMBER;
  V_FECHA_PAGO DATE := SYSTIMESTAMP;
...
BEGIN
  INSERT INTO FIDE_PROLEARN_FINAL_PROF.FIDE_FACTURA_TB (USUARIO_ID, MONTO_ID, FECHA_PAGO)
  VALUES (V_USUARIO_ID, V_MONTO_ID, SYSTIMESTAMP);
  COMMIT;

  BEGIN
    SELECT ROL_ID, NOMBRE INTO V_ROL_ID, V_NOMBRE_USUARIO
    FROM FIDE_PROLEARN_FINAL_PROF.FIDE_USUARIOS_TB
    WHERE USUARIOS_TB_ID_USER_PK = V_USUARIO_ID;
      COMMIT;
  END;
  ...
  BEGIN
    ...
    WHERE MONTO_TB_ID_MT_PK = V_MONTO_ID;
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('No se encontró monto con ID ' || V_MONTO_ID);
      RETURN;
  END;

  V_INTERVALO := V_MONTO_TB.INTERVALO;

  V_FECHA_PAGO := SYSTIMESTAMP;
  V_FECHA_EXPIRACION := V_FECHA_PAGO + V_INTERVALO;

  UPDATE FIDE_PROLEARN_FINAL_PROF.FIDE_FACTURA_TB
  SET FECHA_EXPIRACION = V_FECHA_EXPIRACION
  WHERE USUARIO_ID = V_USUARIO_ID AND ROWNUM = 1;
  COMMIT;
